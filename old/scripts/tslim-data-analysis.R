...
tslim_data <- "data/tslim"
data_fns <- dir(file.path(tslim_data), full.names = TRUE)

#' @note function that finds indices for cgm and bolus data
#' @param fn file path with name and extension, for reading
#' @return tibble containing indices for start and stop of cgm and bolus data
find_csv_data <- function(fn) {
  connection <- file(fn)
  open(connection)

  #' @Note this function should loop through the csv dataset and identify
  #' which parts of the file are cgm and which are bolus
  ...
  found_one_blank <- 0
  while (length(line) > 0) {
    cgm_line <- str_detect(string = line, pattern = "CGM")
    bolus_line <- str_detect(string = line, pattern = "BolusRequestID")

    if (cgm_line) {
      # print(cgm_line)
      # print(line_number)
      cgm_data_start <- line_number - 1
      message(qq("Found CGM data start: @{cgm_data_start}"))
    } else if (bolus_line) {
      # print(bolus_line)
      # print(line_number)
      bolus_data_start <- line_number - 1
      message(qq("Found Bolus data start: @{bolus_data_start}"))
    }
    if (length(line) == 1 & line == "") {
      found_one_blank <- found_one_blank + 1
      # print(qq("    Found a blank at @{line_number}. Number of blanks @{found_one_blank}"))
    }
    if (length(line) == 1 & line == "" & found_one_blank == 2) {
      cgm_data_end <- line_number - 2 - cgm_data_start # one less than the blank is the end of cgm, incorporating start
      message(qq("Found CGM data end: @{cgm_data_end}"))
      found_one_blank <- found_one_blank + 1
    }
  ...
  bolus_data_end <- line_number
  message(qq("Found Bolus data end (end of file): @{bolus_data_end}"))
  ...
  return(tibble(
    fn = fn,
    cgm_data_start = cgm_data_start,
    cgm_data_end = cgm_data_end,
    bolus_data_start = bolus_data_start,
    bolus_data_end = bolus_data_end
  ))
}

#' @note function that reads in bolus data from bounds established in find_csv_data
#' @param ci tibble returned from find_csv_data, containing the start and end
#' indices of the cgm and bolus data
#' @return tibble of that data
read_bolus_data <- function(ci = tibble()) {
  bolus_dat <- read_csv(
    file = ci$fn[1],
    skip = ci$bolus_data_start[1],
    n_max = ci$bolus_data_end[1],
    show_col_types = FALSE
  ) %>%
    dplyr::select(
      Description, BG, CompletionDateTime, FoodBolusSize, CorrectionBolusSize,
      ActualTotalBolusRequested, CompletionStatusDesc, BolusType,
      UserOverride, TargetBG, CorrectionFactor, CarbSize,
      EventHistoryReportDetails, NoteID
    ) %>%
    separate(col = NoteID, into = c("CorrectionFactor1", "ICR", "TargetBG1"), sep = " - ") %>%
    mutate(InsulinToCarbRatio = str_extract(string = ICR, pattern = "[0-9]:[0-9][0-9]*")) %>%
    dplyr::select(-c("CorrectionFactor1", "TargetBG1", "ICR"))

  return(bolus_dat)
}

#' @note function that reads in cgm data from bounds established in find_csv_data # nolint
#' @param ci tibble returned from find_csv_data, containing the start and end
#' indices of the cgm and bolus data
#' @return tibble of that data
read_cgm_data <- function(ci = tibble()) {
  cgm_dat <- read_csv(
    file = ci$fn[1],
    skip = ci$cgm_data_start[1],
    n_max = ci$cgm_data_end[1],
    show_col_types = FALSE
  ) %>%
    select(-c("DeviceType", "SerialNumber", "Description")) %>%
    rename(BG = `Readings (CGM / BGM)`, DateTime = EventDateTime) %>%
    select(BG, DateTime) %>%
...
temp_df <- tibble(fn = data_fns) %>%
  mutate(
    csv_info = map(.x = fn, .f = find_csv_data),
    cgm_data = map(.x = csv_info, .f = read_cgm_data),
    bolus_data = map(.x = csv_info, .f = read_bolus_data)
  ) %>%
  suppressWarnings() # annoying warnings... don't mean anything seemingly
